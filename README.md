...
A small coin-flip puzzle game written in C++ with Qt Widgets. Tap a coin to flip it and its four neighbours; turn every coin on the 4×4 board face-up (gold) to clear the level. There are 100 levels: 20 hand-authored ones followed by 80 generated levels of steadily increasing difficulty.
...
- **Level picker** – a 4×5 grid of numbered level buttons, 20 per page, with `<` / `>` buttons to move between the five pages.
...
| `chooselevelscene.h/.cpp` | Paged level picker; creates a `PlayScene` for the chosen level and handles returning from it. |
...
| `dataconfig.h/.cpp` | Holds the 100 level layouts as a `constexpr` table of 4×4 `bool` boards (`1` = face-up, `0` = face-down), looked up with `GetLevelBoard(level)`. |
...
Level data lives in `dataconfig.cpp` as the `kLevels` table; entry `n - 1` is level `n`, and each inner array is one column of the board from top to bottom. To change a level, edit its entry. To add a level beyond 100, append an entry and bump `kLevelCount` in `dataconfig.h`; the level picker adds pages as needed.

Not every layout is solvable. Flipping a coin toggles a plus-shaped group of up to five coins, and on a 4×4 board only 4096 of the 65536 layouts can be reached from the solved board. Build new layouts by starting from all face-up and applying a set of flips; the number of flips in the shortest such set is the level's difficulty, which tops out at 7 on a 4×4 board. Levels 21–100 were generated this way and sorted from 4-move to 7-move puzzles.
